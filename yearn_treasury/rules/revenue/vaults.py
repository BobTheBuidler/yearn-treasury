...
async def is_v1_vault_fees(tx: TreasuryTx) -> bool:
    ...
    for vault, underlying in v1.items():
        ...
        if tx.to_address != rewards:
            ...
            continue
        strategy: ChecksumAddress = await controller.strategies.coroutine(underlying, block_identifier=tx.block)
        if tx.from_address != strategy:
            logger.debug('from address %s doesnt match strategy %s set on controller %s', tx.from_address.address, strategy, controller)
            continue
        return True
    return False
...
def is_inverse_fees_from_stash_contract(tx: TreasuryTx) -> bool:
    return tx.from_address == "0xE376e8e8E3B0793CD61C6F1283bA18548b726C2e" and tx.to_nickname == "Token: Curve stETH Pool yVault"
