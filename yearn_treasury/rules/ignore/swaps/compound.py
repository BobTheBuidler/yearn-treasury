        ...
        if all(arg in event for arg in ("minter", "mintTokens", "mintAmount")):
            ...
            if (
                ...
                if round(minted, 14) == round(tx.amount, 14):
                    return True
                print(f"Compound deposit ctoken side does not match: {round(minted, 14)}  {round(tx.amount, 14)}")
            # underlying side
            elif tx.to_address == event.address and tx.from_address == minter:
                # TODO: get rid of this rounding when we migrate to postgres
                if round(minted, 14) == round(tx.amount, 14):
                    return True
                print(f"Compound deposit underlying side does not match: {round(minted, 14)}  {round(tx.amount, 14)}")
    return False
        ...
        if all(arg in event for arg in ("redeemer", "redeemTokens", "redeemAmount")):
            ...
            if tx.token == event.address and tx.from_address == redeemer:
                ...
                if round(redeemed, 14) == round(tx.amount, 14):
                    return True
                print(f"Compound withdrawal ctoken side does not match: {round(redeemed, 14)}  {round(tx.amount, 14)}")
            # underlying side
            elif tx.to_address == redeemer and tx.from_address == event.address:
                # TODO: get rid of this rounding when we migrate to postgres
                if round(redeemed, 14) == round(tx.amount, 14):
                    return True
                print(f"Compound withdrawal underlying side does not match: {round(redeemed, 14)}  {round(tx.amount, 14)}")
    return False
