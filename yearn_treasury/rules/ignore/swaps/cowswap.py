    ...
    for trade in tx.events["Trade"]:
        if (
            ...
            if (
                ...
                buy_amount = round(token.scale_value(trade["buyAmount"]), 12)
                if round(amount, 12) == buy_amount:
                    return True
                print(f"Cowswap buy amount does not match: {round(amount, 12)}   {buy_amount}")
            # sell side
            elif token_address == trade["sellToken"] and tx.from_address == trade["owner"]:
                # TODO get rid of this rounding when we move to postgres
                sell_amount = round(token.scale_value(trade["sellAmount"]), 12)
                if round(amount, 12) != sell_amount:
                    print(f"Cowswap sell amount does not match: {round(amount, 12)}   {sell_amount}")
                    continue
