...
yearn-treasury is always compiled. PyPI wheels are compiled, setup compiles the project, and tests/CI must validate compiled extensions. Do not invent or reason from an interpreted `.py` runtime path; tracked `.py` files are mypyc source inputs.
...
14) Local `pip install .` generates `build/`; clean it up before closing a worktree to avoid a dirty state.
